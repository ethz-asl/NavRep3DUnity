...
public class TrialRegularRobot : TrialRobot
{
    ...
    public override Robot createRobotComponnent(GameObject agentObject, uint id)
    {
        ...
        // find rigidbody in baselink and remove all momentum
        for (int i = 0; i < agentObject.transform.childCount; i++)
        {
            GameObject child = agentObject.transform.GetChild(i).gameObject;
            Rigidbody rb = child.GetComponent<Rigidbody>();
            if (rb != null) {
                // Debug.Log("Voiding momentum");
