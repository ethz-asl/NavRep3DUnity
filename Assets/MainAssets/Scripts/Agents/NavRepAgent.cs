...
/// <summary>
/// NavRep agent following a controlLaw and a simulation
/// </summary>
public class NavRepAgent : Agent
{
...
/// </summary>
public class TrialNavRepAgent : TrialAgent
{
    [XmlAttribute]
    ...
    {
        NavRepAgent a =agentObject.AddComponent<NavRepAgent>();
        a.id = id;

        float SQUARE_WIDTH = 10.0f + Random.value * 5.0f;
        agentObject.transform.position = new Vector3((Random.value - 0.5f) * SQUARE_WIDTH,
                                                     0.06f,
                                                     (Random.value - 0.5f) * SQUARE_WIDTH);
        Position.vect = agentObject.transform.position;
        Vector3 goal = new Vector3((Random.value - 0.5f) * SQUARE_WIDTH,
                                    0.06f,
                                    (Random.value - 0.5f) * SQUARE_WIDTH);
        ...
        // Init control law
        LawWaypoints newLaw = new LawWaypoints();
        newLaw.reachedDist = 1.0f;
        newLaw.goals.Add(new ConfigVect(-agentObject.transform.position));
        newLaw.goals.Add(new ConfigVect(agentObject.transform.position));
        newLaw.goals.Add(new ConfigVect(goal));
        controlLaw = newLaw;
    ...
    {
        // Debug.Log("getStartingPosition Invoked!");
        // Debug.Log(Position.vect);
